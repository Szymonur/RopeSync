...
export const useSyncManager = () => {
    ...
    const syncAscents = useCallback(async () => {
        ...
        try {
            ...
            const unsynced = await repository.getUnsyncedAscents(Number(currentUserId));
            
            if (unsynced.length === 0) return;

            console.log(`Synchronizacja: Znaleziono ${unsynced.length} przejść do wysłania.`);
            ...
            for (const ascent of unsynced) {
                try {
                    ...
                    console.log(`Zsynchronizowano przejście: ${ascent.id_przejscia}`);
                } catch (error) {
                    console.error(`Błąd synchronizacji przejścia ${ascent.id_przejscia}:`, error);
                    // Kontynuujemy z następnym, ten zostanie na kolejną próbę
                }
            }
        } catch (error) {
