...
const AscentsFilters = ({ visible, onClose, onApply, currentFilters }: AscentsFiltersProps) => {
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.footer}>
                    <TouchableOpacity onPress={handleReset} style={styles.resetButton}>
                        <ThemedText style={{ color: Colors.error }}>Wyczyść</ThemedText>
                    </TouchableOpacity>
                    <ThemedButton onPress={handleApply} style={styles.applyButton}>
                        <ThemedText style={{ color: "white", fontWeight: "bold", textAlign: "center" }}>Zastosuj</ThemedText>
                    </ThemedButton>
                </View>
    ...
    // 2. RENDEROWANIE WARUNKOWE W ZALEŻNOŚCI OD PLATFORMY

    // Jeśli jesteśmy na Webie, zwracamy płaski komponent (ukrywamy go tylko, jeśli visible === false)
    if (isWeb) {
        if (!visible) return null;
        return FilterContent;
    }

    // Jeśli jesteśmy na Mobile (iOS/Android), pakujemy treść w Modal i przyciemniane tło
    return (
        <Modal 
            visible={visible}
            transparent
...
const styles = StyleSheet.create({
	...
	webContainer: {
        flex: 1, // Na webie zajmuje 100% wysokości kontenera po lewej stronie
        padding: 0,
